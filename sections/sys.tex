...
Because the fit involves varying the overall normalization of WZ + $b$, WZ + $c$ and WZ + light templates over the regions described in Section \rec{subsec:regions}, the modelling of these templates impacts the final result. Therefore, additional signal uncertainties, probing the shape of the signal templates as well as the rate of migrations between the number of truth-jets and reconstructed jets, are estimated by comparing estimates from the nominal Sherpa WZ samples with alternate WZ samples generated with \textsc{Powheg}+\textsc{Pythia8}. Separate systematics are included in the fit for WZ + $b$, WZ + $c$ and WZ + light, where the distribution among each of the fit regions is varied based on the prediction of the Powheg sample.
