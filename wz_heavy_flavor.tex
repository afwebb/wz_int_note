...
\subsubsection{Changes relative to v11}
\begin{itemize}
  \item Added feynman diagrams of WZ+b
  \item Updated text to latest fit results (WZ+c now has asymmetric errors)
  \item Removed yield numbers from some plots to avoid overlap with the legend
  \item Fixed various typos, grammar issues
\end{itemize}

\subsubsection{Changes relative to v10}
\begin{itemize}
  \item Included Zjets plots with lep Pt, eta split by flavor
  \item Clarified the text on a few points regarding systematics, results
  \item Fixed an outdated yield table, combined rare top with ttbar
  \item resolved a high Vgamma uncertainty caused by a single large weight event
  \item included correlation plots for the tZ BDT inputs
\end{itemize}

\subsubsection{Changes relative to v9}
\begin{itemize}
  \item Reordered note to put fiducial region definition in the intro, tZ BDT before the event selection
  \item moved alternative fit strategies from the appendix into the main text of the note
  \item removed correlation matrix plot from results
  \item added c/light rejection rates for each DL1r WP
  \item updated OR procedure - previously had an outdated algorithm
  \item Updated results section to include latest fits, table summarizing results
  \item Replaced correlation table with a plot
\end{itemize}

\subsubsection{Changes relative to v8}
\begin{itemize}
  \item Included more references to appendices in the text
  \item Expanded explanation of fiducial region definition
  \item Previous draft claimed that both standard and custom PLVs were used. Text is fixed to state that a custom PLV is used for lepton iso, but standard lepton id is used
  \item Included plots of PLV output, included WPs used
  \item specified that non-prompt CR plots are post correction
  \item changed title of results section
\end{itemize}
...
\section{Results}
\label{sec:results}
\input{sections/truthJetResults}
%-------------------------------------------------------------------------------    
...
%\label{sec:incFit}
%\input{sections/incFit}

%\subsection{Alternate tZ Inclusive Fit}
%\label{sec:inc_tZ}
%\input{sections/with_tZ}

\subsection{DSID list}
\label{sec:dsid_list}
\input{sections/tables/allDSIDs}
